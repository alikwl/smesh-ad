import{s as _,e as A}from"./supabase.M5dTTGS6.js";document.addEventListener("DOMContentLoaded",async()=>{const r=document.getElementById("tab-btn-leads"),l=document.getElementById("tab-btn-contacts"),x=document.getElementById("container-resource-leads"),g=document.getElementById("container-contact-submissions"),b=document.getElementById("leads-table-body"),m=document.getElementById("contacts-table-body"),a=document.getElementById("brief-modal"),v=document.getElementById("brief-modal-name"),y=document.getElementById("brief-modal-service"),f=document.getElementById("brief-modal-content"),E=document.getElementById("brief-modal-close"),h=document.getElementById("brief-modal-ok"),$=document.getElementById("export-csv-btn");let u=[],i=[],p="leads";r?.addEventListener("click",()=>{p="leads",r.classList.add("active","border-[#1D9E75]","text-[#1D9E75]"),r.classList.remove("border-transparent","text-slate-450"),l?.classList.remove("active","border-[#1D9E75]","text-[#1D9E75]"),l?.classList.add("border-transparent","text-slate-450"),x?.classList.remove("hidden"),g?.classList.add("hidden")}),l?.addEventListener("click",()=>{p="contacts",l.classList.add("active","border-[#1D9E75]","text-[#1D9E75]"),l.classList.remove("border-transparent","text-slate-450"),r?.classList.remove("active","border-[#1D9E75]","text-[#1D9E75]"),r?.classList.add("border-transparent","text-slate-450"),g?.classList.remove("hidden"),x?.classList.add("hidden")});const B=async()=>{if(!b||!m)return;const{data:o}=await _.from("resources").select("slug, title"),n={};(o||[]).forEach(t=>{n[t.slug]=t.title});const{leads:c,contacts:d}=await A();u=c||[],i=d||[],u.length===0?b.innerHTML=`
          <tr>
